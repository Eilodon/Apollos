...
def receive_hard_stop(ws: Any, max_messages: int = 20) -> dict[str, Any]:
    for _ in range(max_messages):
        payload = ws.receive_json()
        if payload.get('type') == 'HARD_STOP':
            return payload
    raise RuntimeError('Timed out waiting for HARD_STOP payload in websocket stream.')
...
def run(args: argparse.Namespace) -> int:
    ...
    with TestClient(backend_main.app) as client:
        with client.websocket_connect(emergency_path) as ws:
            ...
            if ready.get('type') not in {'emergency_ready', 'heartbeat_ack'}:
                print(f'FAIL: unexpected emergency handshake payload: {ready}')
                return 1
    ...
    print('\n=== ASGI HARD_STOP Benchmark Summary ===')
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description='Benchmark HARD_STOP latency in-process via FastAPI TestClient.')
