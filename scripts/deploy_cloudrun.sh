...
gcloud run deploy "${SERVICE_NAME}" \
  ...
  --cpu=2 \
  --memory=2Gi \
  --set-env-vars="APP_ENV=production,ENABLE_GEMINI_LIVE=1,GEMINI_API_KEY=${GEMINI_API_KEY},USE_FIRESTORE=1,FIRESTORE_REQUIRED=1,GOOGLE_CLOUD_PROJECT=${PROJECT_ID},WS_AUTH_MODE=oidc,OIDC_REQUIRE_STRICT=1,OIDC_ISSUER=${OIDC_ISSUER},OIDC_AUDIENCE=${OIDC_AUDIENCE},OIDC_JWKS_URL=${OIDC_JWKS_URL},TWILIO_REQUIRED=1,ENABLE_DEV_ENDPOINTS=0,CORS_ALLOW_ORIGINS=${FRONTEND_ORIGIN}"
